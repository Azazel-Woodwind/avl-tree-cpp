...
class KeyValuePair {
   public:
    ...
    bool operator<(const KeyValuePair& other) { return k < other.k; }
};
...
class TreeMap {
   ...
   public:
    ...
    void write(ostream& o) const { tree.write(o); }

    KeyValuePair<Key, Value>* find(const Key& key) {
        TreeNode<KeyValuePair<Key, Value>>* found =
            tree.find(KeyValuePair<Key, Value>(key));
        if (found) {
            return &(found->data);
        } else {
            return nullptr;
