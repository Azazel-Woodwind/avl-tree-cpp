...
class BinarySearchTree {
   ...
   public:
    ...
    int maxDepth() { return root->maxDepth(); }
    ...
    void write(ostream& os) const { root->write(os); }
    ...
    TreeNode<T>* insert(T data) {
        ...
        if (res->parent->data < res->data &&
            res->parent->parent->data < res->parent->data) {
            if (res->parent->parent == root.get()) {
                root.release();
                root.reset(res->parent);
            }
            res->parent->parent->rotateLeft();
        } else if (res->data < res->parent->data &&
                   res->parent->data < res->parent->parent->data) {
            if (res->parent->parent == root.get()) {
                root.release();
                root.reset(res->parent);
            }
            res->parent->parent->rotateRight();
        } else if (res->parent->data < res->data &&
                   res->parent->data < res->parent->parent->data) {
            if (res->parent->parent == root.get()) {
                root.release();
                root.reset(res);
    ...
    TreeNode<T>* find(T data) { return findHelper(data, root.get()); }
};
